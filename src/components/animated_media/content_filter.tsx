...
export default function ContentFilter() {
    ...
    return (
        ...
        >
            <div className="bg-slate-600 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 hover:scale-105">
                ...
                <div className="bg-slate-300 w-full h-fit p-1 flex gap-2 items-center">
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        <div className="h-2 w-8 rounded-full bg-slate-400" />
                    </div>
                </div>

                {/* content */}
                <div className="w-48 h-48 bg-slate-100 overflow-hidden relative">
                    <div className={`absolute inset-0 transition-all duration-700 blur-sm `}>
                        <div className="flex">
                            <div className="h-12 w-12 bg-primary" />
                            <div className="h-12 w-12 bg-primary/80" />
                            <div className="h-12 w-12 bg-primary" />
                            <div className="h-12 w-12 bg-primary/40" />
                        </div>
                        <div className="flex">
                            <div className="h-12 w-12 bg-primary/40" />
                            <div className="h-12 w-12 bg-primary" />
                ...
                <div className="bg-slate-300 w-full h-fit p-1 flex gap-2 items-center">
                    ...
                    <div className="h-3 w-5 pr-1 rounded-full bg-slate-400" />
                    <MessageCircleMore className="w-6 h-6 text-slate-500" />
                    <div className="h-3 w-5 pr-1 rounded-full bg-slate-400" />
            ...
            {/* Arrow Animation */}
            <div className="h-2 w-32 relative overflow-hidden">
                {/* {isLoading && ( */}
            ...
            <div className={` relative overflow-hidden rounded-3xl bg-primary flex flex-col items-center justify-start gap-5 py-10 px-6 min-w-64 transition-all duration-500 ${isLoading ? '' : ''}`}>

                <div className={`absolute rounded-3xl overflow-hidden z-10  top-1/2 -translate-y-1/2 ${isLoading ? "w-64" : "w-0"} transition-all duration-700 h-full bg-primary/50 backdrop-blur-sm text-white font-bold flex flex-col items-center justify-center gap-2 text-xl`}>
                ...
                {categories.map((category, index) => (
                    <div key={index} className={`w-full ${isLoading ? "animate-pulse" : "animate-slideIn"}`} style={{ animationDelay: `${index * 100}ms` }}>
                        <div className=" w-full h-9 bg-white rounded-full overflow-hidden relative">
                            <div className=" relative z-10 px-2 h-full flex justify-between items-center text-primary ">
                                <span className={` ${isLoading ? "text-white bg-white" : " bg-transparent text-black"} font-bold transition-all duration-300 rounded-full `}>{category.name}</span>
                                <span className={` ${isLoading ? "text-white bg-white" : " bg-transparent text-black"} font-bold transition-all duration-300 rounded-full `}>{category ? category.percentage : 0}%</span>

                            </div>
                            <div
                                className={`absolute top-0 h-9 rounded-full transition-all duration-1000 ease-out ${category.color}`}
                                style={{
